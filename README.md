# Hello, I'm Bashar Alhato! 🎯

#### Full-Stack Developer from Palestine.
I'm passionate about web development and am always on the lookout for new challenges and opportunities to push my skills and create exceptional work.

## 🚀 Technologies & Tools

Throughout my journey, I have gained experience with the following technologies:  

- **Languages**: PHP, JavaScript, TypeScript
- **Frameworks**: Laravel, Vue.js, React.js, Inertia.js, Livewire, Alpine.js.
- **Databases**: MySQL, PostgreSQL, Redis, NoSQL.  
- **Styling**: Shadcn, Tailwind CSS, SASS
- **DevOps**: CI/CD, AWS, Docker, Nginx, Git, GitHub Actions, Linux.
- **Testing**: TDD, Pest, PHPUnit.  
- **Tools**: Vite, PhpStorm, VS Code, Postman, Figma.
<!-- GitHub Readme Stats -->
[![My GitHub Stats](https://github-readme-stats.vercel.app/api?username=basharalhato&cache_seconds=3600&show_icons=true&theme=vue-dark&hide_title=true)](https://github.com/basharalhato)
<span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
[![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=basharalhato&layout=compact&theme=vue-dark)](https://github.com/basharalhato)
